...
def write_log(de):
	...
	env = '_E_D_A_Z'
	dim = 39
	ctxt_ls = [6,4,2,8,1,0,10]	
	
	#acoustic model parameters
	layers = [1,2,3,4,5,6,7,8,9]
	hidunits = [500]
	ptdecay = ['0.001']
	ftdecay = ['0.001','0.002','0.003','0.01','0.00001','0.1','0.0001','0.0005','0.005','0.05']
	phone = ['xwtri','mono','xwbir','xwbil']
	
	
	#language model parameters
	insword_pl = [0.0,-2.0,-1.0,-4.0,-8.0,-16.0]
	insword_bi = [0.0,-2.0,-1.0,-4.0,-8.0,-16.0]
	grammarscale = [0.0,2.0,1.0,4.0,8.0,16.0]
	...
	f_cv_tr.write('ph_model,nhid,ctxt,layers,ptdecay,ftdecay,cv_acc,tr_acc\n')
	write_str = ''
				...
				for layer in layers:
					for ptl2 in ptdecay:
						for ftl2 in ftdecay:
							...
							write_str =  ph+','+str(hid)+',' +str(ctxt)+','+ str(layer)+','+str(ptl2)+','+str(ftl2)
	...
	f_pl = open(fname,'w')
	f_pl.write('ph_model,nhid,ctxt,layers,ptdecay,ftdecay,ins_pl,decode_acc\n')
	write_str = ''
	for ctxt in ctxt_ls:
		for ph in phone:	
			for hid in hidunits:
					...
					for ptl2 in ptdecay:
						for ftl2 in ftdecay:
							for ins in insword_pl:
								write_str = ph+','+str(hid)+',' +str(ctxt)+',' + str(layer)+','+str(ptl2)+','+str(ftl2)+','+str(ins)
								#string encapsulating everything						
								addr = ph+'_h'+str(hid)+'_c'+str(ctxt)+'_l' + str(layer)+'_pt'+str(ptl2)+'_ft'+str(ftl2)
	...
	f_bi = open(fname,'w')
	f_bi.write('ph_model,nhid,ctxt,layers,ptdecay,ftdecay,ins,gscale,decode_acc\n')
	write_str = ''
	for ctxt in ctxt_ls:
		for ph in phone:	
			for hid in hidunits:
